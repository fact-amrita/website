...
const TicketTable: React.FC = () => {
  ...
  return (
    <div className="p-4">
      <h2 className="text-2xl font-bold mb-4 text-zinc-50">Ticket List</h2>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {tickets.map(ticket => (
            <tr key={ticket.TicketId} onClick={() => handleTicketClick(ticket)} className="cursor-pointer hover:bg-gray-50">
              ...
              <td className="px-6 py-4 whitespace-wrap break-all">{((ticket.TicketContent).length > 15) ? ((ticket.TicketContent).substring(0, 15) + "...") : (ticket.TicketContent)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      {selectedTicket && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-md p-6 rounded-lg shadow-lg" style={{ zIndex: 1000 }}>
            ...
            {((selectedTicket.TicketType) == "Complaint") && (<p><strong>Ticket Status:</strong> {(selectedTicket.cleared) ? "Cleared" : "Not Cleared"}</p>)}
            ...
            {(!selectedTicket.cleared) && (<button className="mt-4 ml-2 py-2 px-4 bg-green-200 text-gray-800 rounded-md hover:bg-green-300" onClick={async () => { await clearTicket(selectedTicket.TicketId); closeModal(); window.location.reload(); }}>
              Clear Ticket
            </button>)}
